...
class EtcdAdapter {
    ...
    async get(options) {
        ...
        if (!res) {
            throw new Error('failed to get from storage');
        }
        return res;
    }
    ...
    async listPrefixes(options) {
        ...
        const res = await this._etcd.get({ path, isPrefix: true });
        const entries = Object.entries(res);
        const keys = entries.map(x => ({ path: x[0] }));
        return keys.map(r => r.path.match(options.path + '/(.*)/')[1]);
    }
    ...
    async delete(options) {
        ...
        return this._etcd.delete({ path, isPrefix: true });
    }
}
